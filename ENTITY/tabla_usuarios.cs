namespace ENTITY
{
    ...
    public partial class tabla_usuarios
    {
        ...
        public static UT0030 ObtenUsuario(string texto)
        {
            var alumnos = new UT0030();
            
            try
            {
                using (var ctx = new RSFACAR())
                {
                    alumnos = ctx.UT0030.Where(x => x.TU_ALIAS.Trim() == texto.Trim()).FirstOrDefault();

                    if (alumnos == null)
                    {

                        UT0030 alumnos1 = new UT0030()
                        {
                            TU_NOMUSU = "",

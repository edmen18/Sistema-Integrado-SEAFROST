namespace ENTITY
{
    ...
    using System.Linq;
    public partial class VISTA_PEDIDOS
    {
        [Key]
        [Column(Order = 0)]
        [StringLength(11)]
        ...
        public static int cpedidosa(string estad)
        {
            using (RSFACAR context = new RSFACAR())
            {
                var cnp = (from c in context.VISTA_PEDIDOS where c.F5_CESTADO2 == estad select c).Count();
                return cnp;

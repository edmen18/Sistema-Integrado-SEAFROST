namespace ENTITY
{
    ...
    public partial class AL0003TABL
    {
        ...
        public static List<AL0003TABL> Listarmonedasoles(AL0003TABL TABL)
        {
            var alumnos = new List<AL0003TABL>();
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    alumnos = (from c in ctx.AL0003TABL.Where(x => x.TG_CCOD == TABL.TG_CCOD && x.TG_CCLAVE.Trim() == "MN")
                               select new
                               {
                                   TG_CCOD = c.TG_CCOD,
                                   TG_CCLAVE = c.TG_CCLAVE,
        ...
        public static List<AL0003TABL> ListartablaxDetalle(string texto, string codig)
        {
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    info = ctx.AL0003TABL.Where(x => (x.TG_CDESCRI.Contains(texto) || x.TG_CCLAVE.Contains(texto)) && x.TG_CCOD == codig.Trim()).ToList();
                }
            }
            catch (Exception)
            {
        ...
        public static List<FT0003LINE> ListarvALORESParaLinea(string cod1, string cod2)
        {
            var alumnos = new List<FT0003LINE>();
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    ...
                    alumnos = ctx.Database.SqlQuery<FT0003LINE>("select * from  FT0003LINE  where LI_CCODLIN between @cod1 and @cod2 order by rtrim(ltrim(LI_CCODLIN))",
        ...
        public static List<AL0003TABL> ListarCentrosCosto(string cod1, string cod2)
        {
            var alumnos = new List<AL0003TABL>();
            try
            {
                using (var ctx = new RSFACAR())
                {
                    alumnos = ctx.Database.SqlQuery<AL0003TABL>("select * from  AL0003TABL  where TG_CCLAVE between @cod1 and @cod2 order by rtrim(ltrim(TG_CDESCRI))",
                           new SqlParameter("cod1", cod1), new SqlParameter("cod2", cod2)).ToList();
                }
            }
            catch (Exception)
        ...
        public static List<FT0003LINE> ListarLineas(string texto)

        {
            var LINEA = new List<FT0003LINE>();

            try
            {
                using (var ctx = new RSFACAR())
                {
                    LINEA = (from c in ctx.FT0003LINE
                             where (c.LI_CCODLIN.Contains(texto) || c.LI_CDESLIN.Contains(texto))
                             select new
                             {
                                 LI_CCODLIN = c.LI_CCODLIN,
                                 LI_CDESLIN = c.LI_CDESLIN,
                             }

                        ).ToList().Select(c => new FT0003LINE()
                        {
                            LI_CDESLIN = c.LI_CDESLIN.Trim(),
                            LI_CCODLIN = c.LI_CCODLIN.Trim(),
                        }).ToList(); ;
                }
            }
            catch (Exception)
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    ARTICULOS = (from c in ctx.AL0003TABL
                                 where (c.TG_CDESCRI.Contains(texto) || c.TG_CCLAVE.Contains(texto)) && c.TG_CCOD == INDICADOR
                                 select new
                                 {
                                     TG_CCLAVE = c.TG_CCLAVE,
                                     TG_CDESCRI = c.TG_CDESCRI,
        ...
        public static AL0003TABL obtenvariosED(string texto, string ind)
        {
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    alumnos = ctx.AL0003TABL.Where(x => x.TG_CCLAVE.Trim() == texto.Trim() && x.TG_CCOD== ind).FirstOrDefault();
                    if (alumnos == null)
                    {
        ...
        public static List<AL0003TABL> ListarCCostoParaKardex(string cod1, string cod2)
        {
            var alumnos = new List<AL0003TABL>();
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    alumnos = ctx.Database.SqlQuery<AL0003TABL>("select * from  AL0003TABL where TG_CCOD='10' and TG_CCLAVE between @cod1 and @cod2 order by rtrim(ltrim(TG_CCLAVE))",
                           new SqlParameter("cod1", cod1), new SqlParameter("cod2", cod2)).ToList();
                }
            }
            catch (Exception)

namespace ENTITY
{
    ...
    public partial class UT0030
    {
        ...
        /// <summary>
        /// Metodo para obtener los datos de un usuarios 
        /// Con un arreglo
        /// </summary>
        /// <param name="DATA"></param>
        /// <returns></returns>
        public static List<UT0030> cuentas(string[] DATA)
        {
            var consulta = new List<UT0030>();
            try
            {
                using (var ctx = new RSFACAR())
                {
                    consulta = (from c in ctx.UT0030
                                    where
                                    ((DATA).Contains(c.TU_ALIAS)) 
                                    select c).ToList();

namespace ENTITY
{
    ...
    public partial class AL0003ALMA
    {
        ...
        public static List<AL0003ALMA> ListarAlmacenIDS()
        {
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    ...
                    listaA = (from a in ctx.AL0003ALMA.Where(x => ((new string[] { "A002", "A004", "0004", "0008","0015","0009" }).Contains(x.A1_CALMA)))
                              select new
                              {
                                  A1_CALMA = a.A1_CALMA,
            ...
            using (var ctx = new RSFACAR())
            {
                ...
                Listarep = (from ARTI in ctx.AL0003ARTI
                            ...
                            where
                             ...
                             arreglo.Contains(MOVG.C6_CCODMOV.Trim()) && movimiento.Contains(MOVG.C6_CTIPMOV.Trim())
                             && almacen.Contains(ALMACEN.A1_CALMA.Trim()) && articulo.Contains(ARTI.AR_CCODIGO.Trim())
                             )
                            select new
                            {
                                ...
                                DATA20 = ((from b in ctx.AL0003TABL
                                           ...
                                           select new { b.TG_CDESCRI }).FirstOrDefault().TG_CDESCRI),
                                //familia
                                DATA25 = ((from b in ctx.AL0003TABL
                                           where b.TG_CCOD.Trim() == "38" && b.TG_CCLAVE.Trim() == ARTI.AR_CFAMILI
                                           select new { b.TG_CDESCRI }).FirstOrDefault().TG_CDESCRI),
...

      {
        ...
        public static List<vista_stockvalorizado> RptSVRESUMIDO(vista_stockvalorizado COM, string ind)
        {
...

            {
                Listarep = (from articulo in ctx.AL0003ARTI
                            ...
                            where
                              ((det.VL_CMESPRO.Trim() == COM.SA_CMESPRO.Trim() && det.VL_NACTCAN != 0
                              )
                              &&
                              ...
                              (clavemarca).Contains(articulo.AR_CMARCA)))
                            orderby
                              articulo.AR_CDESCRI
                            select new
                            {
        ...
        public static List<vista_consumoccosto> rptconsumo(vista_consumoccosto COM, string ind)
        {
            var Listarep = new List<vista_consumoccosto>();
            string[] clave = COM.TG_CCLAVE.Split(',');
            string[] arreglo = COM.CADENA.Split(',');
            string[] movimiento = COM.MOVIMIENTO.Split(',');

            string[] ccosto = COM.CADENACCOSTO.Split(',');
            string[] clavegrupo = new string[clave.Length];
            string[] clavefamilia = new string[clave.Length];
            string[] clavearticulo = new string[clave.Length];
            string[] clavecuenta = new string[clave.Length];
            string[] clavemodelo = new string[clave.Length];
            string[] clavelinea = new string[clave.Length];
            ...
            if (ind == "PL")
            {
                clavelinea = COM.TG_CCLAVE.Split(',');
            }

            int mess = Convert.ToInt16(COM.mes);
            int annio = Convert.ToInt16(COM.annio);

            using (var ctx = new RSFACAR())

            {
                Listarep = (from art in ctx.AL0003ARTI
                            join MOVG in ctx.AL0003MOVG
                            on new { C6_CCODIGO = art.AR_CCODIGO } equals new { C6_CCODIGO = MOVG.C6_CCODIGO }
                            where
                              (movimiento.Contains(MOVG.C6_CTIPMOV) &&
                                arreglo.Contains(MOVG.C6_CCODMOV)
                                &&
                                MOVG.C6_DFECDOC.Value.Year == annio
                                &&
                               MOVG.C6_DFECDOC.Value.Month == mess && ccosto.Contains(MOVG.C6_CCENCOS)
                                )
                              &&
                               ((clavefamilia).Contains(art.AR_CFAMILI) ||
                              (clavegrupo).Contains(art.AR_CGRUPO) || (clavecuenta).Contains(art.AR_CCUENTA) ||
                              (clavemodelo).Contains(art.AR_CMODELO)
                              || (clavelinea).Contains(art.AR_CLINEA))
                            group new { art, MOVG } by new
                            {
                                art.AR_CCODIGO,
                                art.AR_CDESCRI,
                                MOVG.C6_CCENCOS,
                            ...
                            select new
                            {
                                data1 = g.Key.AR_CCODIGO,
                                data2 = g.Key.AR_CDESCRI,
                                data3 = g.Key.AR_CCUENTA,
                                data4 = g.Key.AR_CFAMILI,
                                data5 = g.Key.AR_CLINEA,
                                data6 = g.Key.AR_CMODELO,
                                data7 = g.Key.AR_CGRUPO,
                                data8 = g.Sum(s => s.MOVG.C6_NCANTID),
                                data9 = g.Sum(s => s.MOVG.C6_NUSIMPO),
                                data10 = g.Sum(s => s.MOVG.C6_NMNIMPO),
                                data11 = g.Key.C6_CCENCOS,


                            }).ToList().
                         Select(c => new vista_consumoccosto()
                         {
                             AR_CCODIGO = c.data1,
                             AR_CDESCRI = c.data2,
                             familia = AL0003ALMA.obtenDATOSVARIOS(c.data4, "38").TG_CDESCRI,
                             grupo = AL0003ALMA.obtenDATOSVARIOS(c.data7, "06").TG_CDESCRI,
                             cuenta = AL0003ALMA.obtenDATOSVARIOS(c.data3, "07").TG_CDESCRI,
                             modelo = AL0003ALMA.obtenDATOSVARIOS(c.data6, "39").TG_CDESCRI,
                             linea = AL0003ALMA.obtenDATOSLINEA(c.data5).LI_CDESLIN,
                             AR_CFAMILI = c.data4,
                             AR_CLINEA = c.data5,
                             AR_CMODELO = c.data6,
                             AR_CCUENTA = c.data3,
                             AR_CGRUPO = c.data7,
                             cantidad = Convert.ToDecimal(c.data8),
                             ImpDolares = Convert.ToDecimal(c.data9),
                             ImpSoles = Convert.ToDecimal(c.data10),
                             C6_CCENCOS = c.data11,
                             centrocosto = AL0003ALMA.obtenDATOSVARIOS(c.data11, "10").TG_CDESCRI,
                         }).ToList();
            }
            return Listarep;
        }

        public static List<vista_consumoccosto> rptconsumoes(vista_consumoccosto COM, string ind)
        {
            var Listarep = new List<vista_consumoccosto>();
            string[] clave = COM.TG_CCLAVE.Split(',');
            string[] arreglo = COM.CADENA.Split(',');
...

            {
                Listarep = (from art in ctx.AL0003ARTI
                            ...
                            select new
                            {
                                ...
                                data6 = g.Key.AR_CMODELO,
                                data7 = g.Key.AR_CGRUPO,
                                data8 = g.Where(s => s.MOVG.C6_CTIPMOV == "S" && s.MOVG.C6_CCENCOS == g.Key.C6_CCENCOS).Sum(s => (decimal?)s.MOVG.C6_NCANTID),
                                data9 = g.Where(s => s.MOVG.C6_CTIPMOV == "E" && s.MOVG.C6_CCENCOS == g.Key.C6_CCENCOS).Sum(s => (decimal?)s.MOVG.C6_NCANTID),
                                data10 = g.Where(s => s.MOVG.C6_CTIPMOV == "S" && s.MOVG.C6_CCENCOS == g.Key.C6_CCENCOS).Sum(s => (decimal?)s.MOVG.C6_NUSIMPO),
                                data11 = g.Where(s => s.MOVG.C6_CTIPMOV == "E" && s.MOVG.C6_CCENCOS == g.Key.C6_CCENCOS).Sum(s => (decimal?)s.MOVG.C6_NUSIMPO),
                                data12 = g.Where(s => s.MOVG.C6_CTIPMOV == "S" && s.MOVG.C6_CCENCOS == g.Key.C6_CCENCOS).Sum(s => (decimal?)s.MOVG.C6_NMNIMPO),
                                data13 = g.Where(s => s.MOVG.C6_CTIPMOV == "E" && s.MOVG.C6_CCENCOS == g.Key.C6_CCENCOS).Sum(s => (decimal?)s.MOVG.C6_NMNIMPO),
                                data14 = g.Key.C6_CCENCOS
                            }).ToList().
                         Select(c => new vista_consumoccosto()
                         {
                             AR_CCODIGO = c.data1,
                             ...
                             AR_CCUENTA = c.data3,
                             AR_CGRUPO = c.data7,
                             cadcantidad = Convert.ToString(c.data8),
                             caddevolucion = Convert.ToString(c.data9),
                             caddolares = Convert.ToString(c.data10),
                             caddevoluciondol = Convert.ToString(c.data11),
                             cadsoles = Convert.ToString(c.data12),
                             caddevolucionsol = Convert.ToString(c.data13),
                             C6_CCENCOS = c.data14,
                             centrocosto = AL0003ALMA.obtenDATOSVARIOS(c.data14, "10").TG_CDESCRI,
                         }).ToList();
            }
            return Listarep;
        }
        public static List<AL0003ALMA> ListarAlmacenAc(string texto)

        {
            var ARTICULOS = new List<AL0003ALMA>();

            try
            {
                using (var ctx = new RSFACAR())
                {
                    ARTICULOS = (from c in ctx.AL0003ALMA
                                 where c.A1_CDESCRI.Contains(texto) || c.A1_CALMA.Contains(texto)
                                 select new
                                 {
                                     A1_CALMA = c.A1_CALMA,
                                     A1_CDESCRI = c.A1_CDESCRI,
                        ...
                        {
                            A1_CALMA = c.A1_CALMA.Trim(),
                            A1_CDESCRI = c.A1_CALMA.Trim() + "-" + c.A1_CDESCRI.Trim(),
                        }).ToList(); ;
                }
            }
            catch (Exception)
        ...
        public static AL0003ALMA VerunAlmacen(string CALMA)
        {
            AL0003ALMA listaA = new AL0003ALMA();
            try
            {
                using (var ctx = new RSFACAR())
                {
                    listaA = (from c in ctx.AL0003ALMA where c.A1_CALMA == CALMA orderby c.A1_CDESCRI select c).FirstOrDefault();
                }
            }
            catch (Exception)
            {

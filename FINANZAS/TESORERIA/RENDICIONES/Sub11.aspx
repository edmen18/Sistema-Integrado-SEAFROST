<%@ Page Title="Comprobantes Compras" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true"
    CodeFile="Sub11.aspx.cs" Inherits="FINANZAS_TESORERIA_RENDICIONES_Sub11" %>
    ...
    select: function (event, ui) {
        ...
        $('#MainContent_txtbancob').val(str);
            ...
            $("#MainContent_txtbancob").autocomplete(
          {
            ...
            //    if((Number(Operacion.mask('lbltotal').text()) + Number(Operacion.mask('txtimportetotal').val()))> Number(Operacion.mask('txtmontodeclarado').val())){
            //        alert("No es posible la operacion ya que el monto ingresado supera al total de la solicitud");                   
            //    }
            //    else {
            ...
            function tipocambioVenta() {
                ...
                $.ajax({
                    ...
                    type: "POST",
                    url: "Sub11.aspx/obetenertcambiocvEdgar",
     ...
     <script type="text/javascript">
         $(function () {
             ...
             function ConsultaUnog(id) {
              ...
              function insertadetcomprobante(DCOMPRO) {
                ...
                for (i = 1; i < gridView.rows.length; i++) {
                    ...
                    DETADC.DCODANE = (gridView.rows[i].cells[1].value == "GA" ? DCOMPRO.ANEXO : gridView.rows[i].cells[1].value == "OB" ? DCOMPRO.CODIGO_PROVEEDOR : " "); //ConsultaDatosCuenta(gridView.rows[i].cells[1].innerHTML).anexocuenta 
                    ...
                    DETADC.DFECDOC = f.getFullYear().toString().substr(2, 2) + (Number(f.getMonth() + 1) < 10 ? "0" + (f.getMonth() + 1) : (f.getMonth() + 1)) + f.getDate();

                    DETADC.DFECVEN = (gridView.rows[i].cells[1].value == "OB" ? f.getFullYear().toString().substr(2, 2) + (Number(f.getMonth() + 1) < 10 ? "0" + (f.getMonth() + 1) : (f.getMonth() + 1)) + f.getDate() : "      ");
                    DETADC.DAREA = "";
                    DETADC.DFLAG = "S";
                    ...
                    DETADC.DFECCOM2 = f;//(moment(f, "DD/MM/YYYY"));
                    DETADC.DFECDOC2 = f;//(moment(f, "DD/MM/YYYY"));
                    DETADC.DFECVEN2 = (gridView.rows[i].cells[1].value == "OB" ? f : null);//(gridView.rows[i].cells[1].val == "OB"?(moment(f, "DD/MM/YYYY")) : null);
                    DETADC.DCODANE2 = "";
                    DETADC.DVANEXO = (gridView.rows[i].cells[1].value == "GA" ? DCOMPRO.DVANEXO : gridView.rows[i].cells[1].value == "OB"? ConsultaDatosCuenta(gridView.rows[i].cells[1].innerHTML).anexocuenta.trim() : "" );
                    DETADC.DVANEXO2 = "";
                    DETADC.DTIPCAM = 0;
                    DETADC.DCANTID = 0;
                    ...
                    $.ajax({
                        type: "POST",
                        url: "Sub11.aspx/InsertDetComprobante",
             ...
             function InsertarCabComprobante(CABECERA) {
                ...
                $.ajax({
                    type: "POST",
                    url: "Sub11.aspx/InsertCabComprobante",
                    ...
                    success: function (response) {
                        ...
                        InsertaBalh();
                        ActComprobante(Number((DETACAB.CCOMPRO).substr(2, 4)), DETACAB.CSUBDIA);
                    },
                    error: function (response) {
                        if (response.length != 0)
                            alert(response);
             ...
             function ActComprobante(numero, subd) {
                 var DETA = {};
                 var fecha = new Date();
                 DETA.CTSUBDIA = subd;
                 ...
                 DETA.CTNUMER = Number(numero.toString());
                                  
                 $.ajax({
                     type: "POST",
                     url: "Sub11.aspx/Numeracion",
          ...
          function InsertaBalh()
            {
             ...
             function ConsultaDatosCuenta(cuenta) {
                 ...
                 $.ajax({
                     type: "POST",
                     url: "Sub11.aspx/ConsultaUnaCuenta",
                     data: '{descri: ' + cuenta + '}',
                     contentType: "application/json; charset=utf-8",
                     dataType: "json",
                     async: false,
             ...
             function ActualizaEstado() {
                 ...
                 $.ajax({
                     type: "POST",
                     url: "Sub11.aspx/CambiarEstado",
                     data: '{COM: ' + JSON.stringify(tabla) + '}',
                     contentType: "application/json; charset=utf-8",
                     dataType: "json",
                     async: false,
                     success: function (response) {
                         ActComprobante(DETACAB.CCOMPRO, DETACAB.CSUBDIA);
                     },
                     error: function (response) {  
                         if (response.length != 0)
                             alert(response);
...
<asp:Content ID="BodyContent" runat="server" ContentPlaceHolderID="MainContent">
    ...
    <div id="Asiento" >
        ...
        <table>
            <tr>
                <td>
                    <div style="overflow: auto; width: 650px; height: 200px">

    ...
    public partial class AL0003MOVD
    {
        ...
        {   //VALORIZA PARTES MANUALES - LIQUIDACIONES DE COMPRA
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    ...
                    data.C6_NVALTOT = VDETA.C6_NVALTOT;
                    data.C6_NMNIMPO = VDETA.C6_NMNIMPO;
                    data.C6_NUSIMPO = VDETA.C6_NUSIMPO;
        ...
        public static List<AL0003MOVD> Detalleagrupaentradasa(string fdoc)
        {
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    ctx.AL0003SKNU.RemoveRange(ctx.AL0003SKNU.Where(x => x.SA_CMESPRO == periodoproceso));
                    ctx.SaveChanges();
                }
            }
            catch { }
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    datos = (from t in ctx.AL0003MOVD
                             where
                               t.C6_CCODIGO == DING.C6_CCODIGO && t.C6_CTD == "PE"

namespace ENTITY
{
    ...
    public partial class tabla_d_usuaprod
    {
        ...
        public static int NumAprob(tabla_d_usuaprod ADDMC)
        {
            ...
            using (var ctx = new ANEXO_RSFACAR())
            {       
                nreg = (from c in ctx.tabla_d_usuaprod where c.DA_IDUSUA == ADDMC.DA_IDUSUA && c.DA_CODIGO.Trim() == ADDMC.DA_CODIGO.Trim() select c).ToList().Count();
            }
            return nreg;
        }
        ...
        public static int NumAprobTotal(tabla_d_usuaprod ADDMC)
        {
            ...
            using (var ctx = new ANEXO_RSFACAR())
            {
                nreg = (from c in ctx.tabla_d_usuaprod where c.DA_CODIGO.Trim() == ADDMC.DA_CODIGO.Trim() select c).ToList().Count();
            }
            return nreg; 
        }
        ...
        public static void DesapruebaTarifa(tabla_d_usuaprod ADDMC)
        {
            using (var ctx = new ANEXO_RSFACAR())
            {
                ctx.tabla_d_usuaprod.RemoveRange(ctx.tabla_d_usuaprod.Where(x => x.DA_CODIGO == ADDMC.DA_CODIGO.Trim()));
                ctx.SaveChanges();
            }
        }


        public static List<tabla_d_usuaprod> AprobacionxProducto(string ADDMC)
        {
            var nreg = new List<tabla_d_usuaprod>();
            using (var ctx = new ANEXO_RSFACAR())
            {
        ...
        public static List<tabla_d_usuaprod> AprobacionxProductodetall(string ADDMC)
        {
            var nreg = new List<tabla_d_usuaprod>();
            using (var ctx = new ANEXO_RSFACAR())
            {
                //nreg = (from c in ctx.tabla_d_usuaprod where c.DA_CODIGO.Trim() == ADDMC.Trim() select c).ToList();
                nreg = (from c in ctx.tabla_d_usuaprod.Where(x => x.DA_CODIGO == ADDMC.Trim())
                        select new
                        {

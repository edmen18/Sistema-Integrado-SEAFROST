namespace ENTITY
{
    ...
    public partial class tabla_d_areausua
    {
        ...
        [Key]
        [Column(Order = 3)]
        public int UA_TIPOPROCESO { get; set; }
        ...
        public static void EliminaDetalleua(tabla_d_areausua DDET)
        {
            try
            {
                using (var ctx = new ANEXO_RSFACAR())
                {
                    ...
                    ctx.tabla_d_areausua.RemoveRange(ctx.tabla_d_areausua.Where(x => x.UA_USUA == DDET.UA_USUA.Trim() && x.UA_TIPOPROCESO == DDET.UA_TIPOPROCESO));
                    ctx.SaveChanges();
                }
            }
            catch
        ...
        public static List<tabla_subareas> Listarsubareasxusua(string usuario,int tipousua)
        {
        ...
        public static int Listarsubareacceso(string usuario,string subarea,int tipoproc )
        {
            ...
            try
            {
                ...
                {
                    dato = (from a in ctx.tabla_d_areausua 
                            where a.UA_USUA == usuario && a.UA_SUBAREA.ToString()==subarea && a.UA_TIPOPROCESO== tipoproc 

namespace ENTITY
{
    ...
    public partial class CT0003ANEX
    {
        ...
        public static List<CT0003ANEX> ListarProveedoresid(string texto)
        {
            var alumnos = new List<CT0003ANEX>();
            ...
            try
            {
                using (var ctx = new RSCONCAR())
                {
                    alumnos = ctx.CT0003ANEX.Where(x => x.ACODANE.Contains(texto))
                                     .ToList();
                }
            }
            catch (Exception)
        ...
        public static List<CT0003ANEX> listarAnexo1(string cod, string texto)
        {
            var lista = new List<CT0003ANEX>();
            try
            {
                ...
                {
                    //lista = ctx.CT0003ANEX.Where(x=>x.AVANEXO==cod && x.ADESANE.Contains(texto)).ToList();
                    lista = ctx.CT0003ANEX.Where(x => x.AVANEXO == cod && x.ACODANE.Contains(texto)).ToList();
                }
            }
            catch (Exception)
            {
            ...
            try
            {
                using (var ctx = new RSCONCAR())
                {
                    ...
                    lista = ctx.CT0003ANEX.Where(x => x.AVANEXO == ADATA.AVANEXO
                            ...
                            && (SqlFunctions.PatIndex(ADATA.ADESANE, x.ADESANE) > 0 || SqlFunctions.PatIndex(ADATA.ARUC, x.ARUC) > 0)
                            && x.AESTADO == "V").ToList();
                }
            }
            catch (Exception)
        ...
        public static CT0003ANEX obtenANEX(CT0003ANEX DTA)
        {
            var alumnos = new CT0003ANEX();
            ...
            try
            {
                using (var ctx = new RSCONCAR())
                {
                    alumnos = ctx.CT0003ANEX.Where(x => x.ACODANE == DTA.ACODANE && x.AVANEXO==DTA.AVANEXO).FirstOrDefault();
                }
            }
            catch (Exception)
            {
        ...
        public static List<CT0003ANEX> ListarProveedoresporid(string cod, string texto,string i)
        {
            ...
            try
            {
                using (var ctx = new RSCONCAR())
                {
                    if (i=="C")
                    {
                        alumnos = ctx.CT0003ANEX.Where(x => x.ACODANE.Contains(texto) && x.AVANEXO.Trim() == cod.Trim())
                                    .ToList();
                    }
                    if (i == "D")
                    {
                        alumnos = ctx.CT0003ANEX.Where(x => x.ADESANE.Contains(texto) && x.AVANEXO.Trim() == cod.Trim())
                                    .ToList();
                    }
        ...
        public static List<CT0003ANEX> anexor(CT0003ANEX DTA)
        {
            ...
            try
            {
                using (var ctx = new RSCONCAR())
                {
                    alumnos = ctx.CT0003ANEX.Where(x => x.AVANEXO.Trim() == DTA.AVANEXO.Trim()).ToList();
                }
            }
            catch (Exception)
            {

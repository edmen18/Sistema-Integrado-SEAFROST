...
<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="Envases.aspx.cs" Inherits="Default2" %>
    ...
    <script type="text/javascript">
        $(function () {
                         ...
                         $.ajax({
                             ...
                             success: function (data) {
                                 response($.map(data.d, function (item) {
                                     return {
                                         value: item.ENV_CODIGO +"-"+ item.ENV_DESCRIPCION,
                                         id: item.ENV_CODIGO,
            ...
            $(".btdelete").click(function () { //  se debe colocar dentrp de una funcion 
                ...
                trp = $(this).parent().parent();
                id = $("td:eq(1)", trp).html();
                DETA.ENV_DESCRIPCION = id;
                DETA.ENV_ESTADO = 0;

                if ((DETA.ENV_CODIGO == "") || (DETA.ENV_DESCRIPCION == "")) {
                    ...
                    if ($("#MainContent_txtindicador").val() == "A") {
                        $.ajax({
                            type: "POST",
                            url: "Envases.aspx/ActualizaAnticipo",
            ...
            function InsertarSolicitud() {
                ...
                var DETA = {};
                DETA.ENV_DESCRIPCION = $("#MainContent_txtdescripcion").val();
                DETA.ENV_CODIGO = $("#MainContent_lblcodigo").html();
                DETA.ENV_ESTADO = 1;
            ...
            function filtarsolicitudesParametro() {
                var VC = {};
                VC.ENV_CODIGO = $("#MainContent_TXTIDANALISIS").val();
               
                $.ajax({

                    type: "POST",
                    url: "Envases.aspx/LISTARDATOSPARAM",
                    ...
                    success: function (data) {
                        ...
                        if (data.d.length > 0) {
                            ...
                            for (var i = 0; i < data.d.length; i++) {
                                $("td", row).eq(0).html(data.d[i].ENV_CODIGO);
                                $("td", row).eq(1).html(data.d[i].ENV_DESCRIPCION);
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    <fieldset style="background-color: #99CCFF">
        <legend style="font-weight: bold; background-color: #99CCFF;">
            <asp:Label ID="Label9" runat="server" Text="LISTADO DE ENVASES" Font-Bold="True" Font-Size="12pt"></asp:Label></legend>
        ...
        <tr>
            <td>
                <asp:GridView ID="gvordencompra" runat="server" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" AutoGenerateColumns="False">
                    <Columns>
                        <asp:BoundField DataField="ENV_CODIGO" HeaderText="CÓDIGO" />
                        <asp:BoundField DataField="ENV_DESCRIPCION" HeaderText="DESCRIPCION" />

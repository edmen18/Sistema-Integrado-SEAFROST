    ...
    <script type="text/javascript">
        $(function () {
                    ...
                    success: function (data) {
                        if (data.d.length > 0) {
                            jQuery.each(data.d, function (i, val) {
                                $('#MainContent_lblSTOCK').text(parseFloat(val['SK_NSKDIS']).toFixed(2));
                            });
                        } else {
            ...
            var cambiarLABEL = function (AL, CM) {
                if ((AL == "A002" || AL == "A004" || AL == "0004") && (CM == "CO" || CM == "MP")) {
                    addComplete1();
                    addComplete2();
            ...
            function stockCOD(AL, COD, TR) {
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.d.length >= 0) {
                            jQuery.each(data.d, function (i, val) {
                                Operacion.mask('lblSTOCK').text(parseFloat(val['SK_NSKDIS']).toFixed(2));
                            });
                        }/* else {
                            //REALIZAR SEGUIMIENTO
                            alert('El producto elegido, no corresponde al almacen elegido');
            ...
            $('#btnFinalizar').click(function () {
                ...
                if (result) {
                    ...
                    for (var t = 1; t < gridView.rows.length; t++) {
                        ...
                        (MOVID == "PR" && MISER != true ? actualizaSerieS(AL, gitem01, gitem03, gitem04) : "");
                        (MOVID == "PR" && MISER != true ? insertaALSE(AL, pad(gitem00, 4), gitem01, gitem03, gitem04) : "");
                    }
                    $('#btnGuardar').attr('disabled', true);
                    $('#btnagregarItem').attr('disabled', true);
                    $('#btnFinalizar').attr('disabled', true);
                    //Operacion.mask('txtidProducto').attr('disabled', true);
            ...
            function guardarCabecera() {
                ...
                DATA.C5_CGUIFAC = "";//N,S
                DATA.C5_CDESTIN = ((XA == "A002" || XA == "A004") && (XM == "CO" || XM == "MP") ? Operacion.mask('ddlDepa').val().trim() : "");//MAX:4 
                DATA.C5_CDIRENV = "";
                ...
                DATA.C5_CNUMLIQ = "";
                DATA.C5_CORDEN = (Operacion.mask('txtOrdenTrabajo').val() != "" ? Operacion.mask('txtOrdenTrabajo').val() : "");//DER->BAHIA
                DATA.C5_CTIPOGS = "";//N
                ...
                DATA.C5_CGLOSA4 = Operacion.mask('txtGlosa4').val();//DER
                DATA.C5_CVENDE2 = "";
                DATA.C5_CESTDEV = "";
                DATA.C5_CEXTOR = "";
                DATA.C5_CRENOM = "";
        ...
        <fieldset>
            ...
            <table style="width: 95%;">
                ...
                <tr>
                    ...
                    <td colspan="2">
                        <asp:TextBox ID="txtGlosa3" TabIndex="13" runat="server" Width="305px" onkeypress="Operacion.MEKPAT(event,this)"></asp:TextBox>
                    </td>
                    <td>&nbsp;</td>
                </tr>
                <tr>

<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="NCregistro.aspx.cs" Inherits="Default2" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="HeadContent" runat="Server">
        ...
        $(function () {
            ...
            $(".idart").autocomplete({
                              ...
                              select: function (event, ui) {
                                  ...
                                  var sxsoli = strockxusuario(str, $(".ctxtidsoli").val()).ee;
    ...
    <script type="text/javascript">
        ...
        function Mostraruncliente(codigo,tipo) {
            var SDATA = {};
            SDATA.ACODANE=codigo;
            SDATA.AVANEXO = tipo;
            var Desc = "";
            var dirprovee = "";
            $.ajax({
                type: "POST",
                url: "NCregistro.aspx/MostrarunANEXO",
        ...
        function grabardetallegrid() {
         ...
         function MostraDetFactura() {
             var ADATA = {};
             ADATA.F6_CNUMSER = Operacion.mask("txtserie").val();
             ADATA.F6_CTD = Operacion.mask("txttipodoc").val();
             ADATA.F6_CNUMDOC = $(".txtndoc").val();
             $.ajax({
                 type: "POST",
                 url: "NCregistro.aspx/ListarDetallFAC",
                 data: '{ADATA: ' + JSON.stringify(ADATA) + '}',
                 contentType: "application/json; charset=utf-8",
                 dataType: "json",
                 async: false,
                 success: function (data) {
                     if (data.d.length > 0) {
                         ...
                         for (var i = 0; i < data.d.length; i++) {                            
                             ...
                             $("td", rowt).eq(0).html(Number(i) + 1);
                             $("td", rowt).eq(1).val(data.d[i].F6_CCODIGO);
                             $("td", rowt).eq(1).html(data.d[i].F6_CDESCRI);
                             $("td", rowt).eq(2).html(data.d[i].F6_CUNIDAD);
                             $("td", rowt).eq(3).html(data.d[i].F6_NCANTID);
                             $("td:eq(4) :text", rowt).val(data.d[i].F6_NPRECIO);
                             //$("td", rowt).eq(5).html(data.d[i].);
         ...
         function MostrarTcambio(fcha) {
             var dato;
             var fdate = moment(fcha, "DD/MM/YYYY");
             fdate == null ? null : new Date(fdate);
             var TCAM = {};
             TCAM.XFECCAM2 = fdate;
             $.ajax({
                 type: "POST",
         ...
         $(function () {
             $("#MainContent_txtsoli").autocomplete(
                 {
                     source: function (request, response) {
                         $.ajax({
                             url: "NCregistro.aspx/ListarCLIENTE",
                             data: "{ 'cod': 'C',texto:'" + $("#MainContent_txtsoli").val() + "' }",
                             dataType: "json",
                             type: "POST",
                             contentType: "application/json; charset=utf-8",
                             dataFilter: function (data) { return data; },
             ...
             $(".txtndoc").autocomplete(
                 {
                     source: function (request, response) {
                         var ADAR = {};
                         ADAR.F5_CTD = Operacion.mask("txttipodoc").val();
                         ADAR.F5_CNUMSER = Operacion.mask("txtserie").val();
                         ADAR.F5_CNUMDOC = $(".txtndoc").val();
                         ADAR.F5_CCODCLI = Operacion.mask("txtidsoli").val();
                         $.ajax({
                             url: "NCregistro.aspx/ListarFactura",
                             data: '{ ADAR:' + JSON.stringify(ADAR) + '}',
                             dataType: "json",
                             type: "POST",
                             contentType: "application/json; charset=utf-8",
                             dataFilter: function (data) { return data; },
                             success: function (data) {
                                 response($.map(data.d, function (item) {
                                     return {
                                         value: item.F5_CNUMDOC.trim(),
                                         id: item.F5_CNUMDOC.trim(),
                                         ffec: item.F5_DFECDOC,
                                         tmone:item.F5_CCODMON
                                     }
                                 }))
                             },
                             error: function (XMLHttpRequest, textStatus, errorThrown) {
                     ...
                     select: function (event, ui) {
                         var str = ui.item.id;
                         var strmon = ui.item.tmone;
                         var fcc =moment(ui.item.ffec).format("DD/MM/YYYY");
                         $('#MainContent_txtfdoc').val(fcc);
                         var tcam = MostrarTcambio($('#MainContent_txtfdoc').val());
                         Operacion.mask("txttcambio").val(tcam);
                         Operacion.mask("txtmoned").val(strmon);
                         MostraDetFactura();
                     }
                 });
    ...
    <script type="text/javascript">
        $(function () {
            ...
            $(".ctxtidsoli").change(function () {
                ...
                var textdev = Mostraruncliente($(this).val(), 'C').Desc;
                if (textdev != "") {
        ...
        <fieldset>
            ...
            <table>
                ...
                <tr>
                    <td>Motivo</td>
                    <td colspan="6">
                        <asp:DropDownList runat="server" ID="ddltipos" Width="250px"></asp:DropDownList>
                    </td>
                </tr>
                <tr>
                    <td>Cliente</td>
                    <td colspan="6">
                        ...
                        <asp:TextBox ID="txtsoli" runat="server" Width="315" placeholder="CLIENTE" TabIndex="2" onkeydown="Operacion.MEKPAT(event,this);"></asp:TextBox>
                    </td>
                </tr>
                <tr style="text-align:center">
                    <td></td>
                    <td style="border-top:outset;border-left:outset;border-right:medium;border-top-left-radius:5px;border-top-right-radius:5px">TIPO</td>
                    <td style="border-top:outset;border-left:outset;border-right:medium;border-top-left-radius:5px;border-top-right-radius:5px">SERIE</td>
                    <td style="border-top:outset;border-left:outset;border-right:medium;border-top-left-radius:5px;border-top-right-radius:5px">NUM DOC</td>
                    <td style="border-top:outset;border-left:outset;border-right:hidden;border-top-left-radius:5px;border-top-right-radius:5px">FECHA DOC</td>
                    <td style="border-top:outset;border-left:outset;border-right:hidden;border-top-left-radius:5px;border-top-right-radius:5px">T. CAMB</td>
                </tr>
                <tr>
                    <td>Doc. Ref.</td>
                    <td>
                        <asp:TextBox runat="server" ID="txttipodoc" Width="60" MaxLength="2"  />
                    </td>
                    <td>
                        <asp:TextBox runat="server" ID="txtserie" Width="80"  MaxLength="4"  />
                    </td>
                    <td>
                        <asp:TextBox runat="server" class="txtndoc" Width="110"  MaxLength="7"   />
                    </td>
                    <td>
                        <asp:TextBox runat="server" ID="txtfdoc" Width="100" Enabled="false" style="text-align:center" />
                    </td>
                    <td>
                    <asp:TextBox ID="txttcambio" runat="server" Width="60" Enabled="false" style="text-align:right"></asp:TextBox>
                    </td>
                </tr>
                <tr>
                    <td>Moneda:</td>
                    <td colspan="2">
                        <asp:TextBox runat="server" ID="txtmoned" Width="60" Enabled="false"  />
                    </td>
                </tr>
                <tr>
                    <td>Glosa</td>
                    <td colspan="6">
                        <asp:TextBox ID="txttra" runat="server"  Width="415" TabIndex="5" onkeydown="Operacion.MEKPAT(event,this);"></asp:TextBox>
                    </td>
                </tr>
                    ...
                    <td colspan="4">
                        <asp:GridView ID="gvdetalle" class="filtrar" runat="server" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" AutoGenerateColumns="False" Width="709px">
                            <Columns>
                                ...
                                <asp:BoundField DataField="SA_NAPROB" HeaderText="PRODUCTO" ItemStyle-Width="400" />
                                ...
                                <asp:TemplateField HeaderText="PRECIO">
                                    <ItemTemplate>
                                        <div class="clprecio" style="text-align: center">
                                            <input type="text" id="txtprecio" style="width:100px" />
                                        </div>
                                    </ItemTemplate>
                                </asp:TemplateField>

    ...
    public partial class AL0003STOC
    {
        ...
        public static Boolean ingresaStock(AL0003STOC SDATA)
        {
            ...
            try
            {
                using (var ctx = new RSFACAR())
                {
                    SDATA.SK_DFECMOV= DateTime.Now;//NUEVO 18.06.16
        ...
        public static List<AL0003STOC> ListarStockVal(string AL, string COD)
        {   
            List<AL0003STOC> listaA = new List<AL0003STOC>();
            try
            {
                using (var ctx = new RSFACAR())
                {
                    listaA = ctx.AL0003STOC.Where(x =>  x.SK_CCODIGO == COD 
                    && (AL =="-1" ? x.SK_CALMA != AL : x.SK_CALMA == AL )).ToList();
                }
            }
            catch
            {

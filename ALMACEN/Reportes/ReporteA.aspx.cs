...
public partial class ALMACEN_Reportes_ReporteA : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        string idPR = Convert.ToString(Request.QueryString["PR"]);
        ...
        string[] NDOC = new string[1];
        NDOC[0] = idND;
        cabecera = AL0003MOVC.listaFinal(TABL).FirstOrDefault();
        detalle = AL0003MOVC.listadetalleCAB(TABL,NDOC);//AL0003MOVC.listaFinal(TABL);
        ...
        foreach (SCabecera p in detalle)
        {
            item++;
            worksheet.Range("A" + cont.ToString()).Style.Font.FontName = "Draft 17cpi";
            worksheet.Range("A" + cont.ToString()).Style.Font.FontSize = 8;
            worksheet.Range("A" + cont.ToString()).SetValue(CapaNegocio.Cls_Utilidades.llenaConceros(item.ToString(),4))
                .Merge();

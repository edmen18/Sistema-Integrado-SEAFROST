...
public partial class ALMACEN_REPORTES_consumo_ExcelConsumoEnt : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        foreach (vista_consumoccosto b in detalle)
            ...
            if (indicador == "39")
            {
                ...
                worksheet.Range("E5").SetValue("DEL MODELO " + varios1 + " HASTA EL MODELO " + varios2).Style.Font.SetBold(true);
                    ...
                    worksheet.Cell("H" + cont2.ToString()).Value = b.cantidad;
                    worksheet.Cell("I" + cont2.ToString()).Value = (b.cantidad*(-1));
                    worksheet.Cell("J" + cont2.ToString()).Value = "0.00";
                    worksheet.Cell("K" + cont2.ToString()).Value = b.ImpDolares;
                    worksheet.Cell("L" + cont2.ToString()).Value = b.ImpDolares*(-1);
                    worksheet.Cell("M" + cont2.ToString()).Value = "0.00";
                    worksheet.Cell("N" + cont2.ToString()).Value = b.ImpSoles;
                    worksheet.Cell("O" + cont2.ToString()).Value = b.ImpSoles * (-1);
        ...
        worksheet.Cell("G" + (cont2 + 1).ToString()).Value = "0.00";
        worksheet.Cell("H" + (cont2 + 1).ToString()).Value = SUMCANT;
        worksheet.Cell("I" + (cont2 + 1).ToString()).Value = SUMCANT * (-1);
        worksheet.Cell("J" + (cont2 + 1).ToString()).Value = "0.00";
        worksheet.Cell("K" + (cont2 + 1).ToString()).Value = SUMDOL;
        worksheet.Cell("L" + (cont2 + 1).ToString()).Value = SUMDOL * (-1);
        worksheet.Cell("M" + (cont2 + 1).ToString()).Value = "0.00";
        worksheet.Cell("N" + (cont2 + 1).ToString()).Value = SUMMN;
        worksheet.Cell("O" + (cont2 + 1).ToString()).Value = SUMMN * (-1);

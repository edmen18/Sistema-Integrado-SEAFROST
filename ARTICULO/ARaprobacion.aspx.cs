...
public partial class Default2 : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!this.IsPostBack)
        {
            ...
            ddlarea.DataSource = tabla_d_areausua.Listarsubareasxusua(hfusu.Value, 1);
            ddlarea.DataBind();
            ddlarea.Items.Insert(0, new ListItem("[AREA]", ""));

            ddlnegocio.Items.Clear();
            ddlnegocio.DataTextField = "AE_DESC";
            ddlnegocio.DataValueField = "AE_COD";
            ddlnegocio.DataSource = tabla_area.Listararea();
            ddlnegocio.DataBind();
            //ddlnegocio.Items.Insert(0, new ListItem("[AREA]", ""));
            ...
            if (sivalida == 1)
            {
                ...
                {
                    lar = AL0003ARTI.ListarArticulosxestado(ddlfilt.SelectedValue, ddlarea.SelectedValue);
                }
                else { 
                lar = AL0003ARTI.ListarServparaVBF(ddlfilt.SelectedValue, ddlarea.SelectedValue).Where(r => r.AR_NMARGE2 == 1).ToList();
                }
            }
            else
            {
    ...
    public void Listahistorico()
    {
        ...
        dtGetDatae.Columns.Add("OC_CCOMENTA"); 
        dtGetDatae.Rows.Add();
    ...
    [System.Web.Services.WebMethod]
    [ScriptMethod()]
    public static List<tabla_d_usuaprod> listarusuariosaproba(string idproducto,int tcod)
    {
        return tabla_d_usuaprod.AprobacionxProductodetall(idproducto, tcod);
    }
   ...
   [System.Web.Services.WebMethod]
    ...
    {
        return tabla_d_areausua.Listarsubareacceso(codusua, codarea,1);
    }
    ...
    {
        return CT0003PLEM.RegistrounCta(ncuenta);
    }

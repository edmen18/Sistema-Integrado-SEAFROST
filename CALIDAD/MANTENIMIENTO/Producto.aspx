...
<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="Producto.aspx.cs" Inherits="Default2" %>
    ...
    <script type="text/javascript">
        $(function () {
                         ...
                         $.ajax({
                             ...
                             success: function (data) {
                                 response($.map(data.d, function (item) {
                                     return {
                                         value: item.PR_CODIGO +"-"+ item.PR_DESCRIPCION,
                                         id: item.PR_CODIGO,
            ...
            $(".btdelete").click(function () { //  se debe colocar dentrp de una funcion 
                ...
                trp = $(this).parent().parent();
                id = $("td:eq(1)", trp).html();
                DETA.PR_DESCRIPCION = id;
                DETA.PR_ESTADO = 0;

                if ((DETA.PR_CODIGO == "") || (DETA.PR_DESCRIPCION == "")) {
                    ...
                    if ($("#MainContent_txtindicador").val() == "A") {
                        $.ajax({
                            type: "POST",
                            url: "Producto.aspx/ActualizaAnticipo",
            ...
            function InsertarSolicitud() {
                ...
                var DETA = {};
                DETA.PR_DESCRIPCION = $("#MainContent_txtdescripcion").val();
                DETA.PR_CODIGO = $("#MainContent_lblcodigo").html();
                DETA.PR_ESTADO = 1;
            ...
            function filtarsolicitudesParametro() {
                var VC = {};
                VC.PR_CODIGO = $("#MainContent_TXTIDANALISIS").val();
               
                $.ajax({

                    type: "POST",
                    url: "Producto.aspx/LISTARDATOSPARAM",
                    ...
                    success: function (data) {
                        ...
                        if (data.d.length > 0) {
                            ...
                            for (var i = 0; i < data.d.length; i++) {
                                $("td", row).eq(0).html(data.d[i].PR_CODIGO);
                                $("td", row).eq(1).html(data.d[i].PR_DESCRIPCION);
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    <fieldset style="background-color: #99CCFF">
        <legend style="font-weight: bold; background-color: #99CCFF;">
            <asp:Label ID="Label9" runat="server" Text="LISTADO DE PRODUCTOS" Font-Bold="True" Font-Size="12pt"></asp:Label></legend>
        ...
        <tr>
            <td>
                <asp:GridView ID="gvordencompra" runat="server" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" AutoGenerateColumns="False">
                    <Columns>
                        <asp:BoundField DataField="PR_CODIGO" HeaderText="CÓDIGO" />

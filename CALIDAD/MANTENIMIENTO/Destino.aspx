...
<%@ Page Title="" Language="C#" MasterPageFile="~/Site.master" AutoEventWireup="true" CodeFile="Destino.aspx.cs" Inherits="Default2" %>
    ...
    <script type="text/javascript">
        $(function () {
                         ...
                         $.ajax({
                             ...
                             success: function (data) {
                                 response($.map(data.d, function (item) {
                                     return {
                                         value: item.DST_CODIGO +"-"+ item.DST_DESCRIPCION,
                                         id: item.DST_CODIGO,
            ...
            $(".btdelete").click(function () { //  se debe colocar dentrp de una funcion 
                ...
                trp = $(this).parent().parent();
                id = $("td:eq(1)", trp).html();
                DETA.DST_DESCRIPCION = id;
                DETA.DST_ESTADO = 0;

                if ((DETA.DST_CODIGO == "") || (DETA.DST_DESCRIPCION == "")) {
                    ...
                    if ($("#MainContent_txtindicador").val() == "A") {
                        $.ajax({
                            type: "POST",
                            url: "Destino.aspx/ActualizaAnticipo",
            ...
            function InsertarSolicitud() {
                ...
                var DETA = {};
                DETA.DST_DESCRIPCION = $("#MainContent_txtdescripcion").val();
                DETA.DST_CODIGO = $("#MainContent_lblcodigo").html();
                DETA.DST_ESTADO = 1;
            ...
            function filtarsolicitudesParametro() {
                var VC = {};
                VC.DST_CODIGO = $("#MainContent_TXTIDANALISIS").val();
               
                $.ajax({

                    type: "POST",
                    url: "Destino.aspx/LISTARDATOSPARAM",
                    ...
                    success: function (data) {
                        ...
                        if (data.d.length > 0) {
                            ...
                            for (var i = 0; i < data.d.length; i++) {
                                $("td", row).eq(0).html(data.d[i].DST_CODIGO);
                                $("td", row).eq(1).html(data.d[i].DST_DESCRIPCION);
...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="Server">
    <fieldset style="background-color: #99CCFF">
        <legend style="font-weight: bold; background-color: #99CCFF;">
            <asp:Label ID="Label9" runat="server" Text="LISTADO DE DESTINOS" Font-Bold="True" Font-Size="12pt"></asp:Label></legend>
        ...
        <tr>
            <td>
                <asp:GridView ID="gvordencompra" runat="server" BackColor="White" BorderColor="#CCCCCC" BorderStyle="None" BorderWidth="1px" AutoGenerateColumns="False">
                    <Columns>
                        <asp:BoundField DataField="DST_CODIGO" HeaderText="CÓDIGO" />
                        <asp:BoundField DataField="DST_DESCRIPCION" HeaderText="DESCRIPCION" />
